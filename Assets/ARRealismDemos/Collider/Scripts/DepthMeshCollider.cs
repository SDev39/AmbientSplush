...
public class DepthMeshCollider : MonoBehaviour
{
    ...
    private int m_UVFromDepthHandle;
    ...
    private void InitializeMesh()
    {
        ...
        m_UVs = new Vector2[m_NumElements];

		float uOffset = 1.0f / (m_MeshWidth - 1);
		float vOffset = 1.0f / (m_MeshHeight -1 );
        ...
        for (int y = 0; y < m_MeshHeight; y++)
        {
            for (int x = 0; x < m_MeshWidth; x++)
            {
				...
				Vector2 uv = new Vector2(vOffset * y, (1 - uOffset * x));
    ...
    private void InitializeComputeShader()
    {
        ...
        m_UVFromDepthHandle = DepthProcessingCS.FindKernel("UVFromDepth");
        ...
        m_UVBuffer = new ComputeBuffer(m_NumElements, sizeof(float) * 2);
    ...
    private void UpdateMesh()
    {
        ...
        if (Render)
        {
			...
			m_Mesh.GetUVs(0, newUV);
			Debug.Log(newUV.Count);
			// m_UVBuffer.GetData(m_UVs);
			int[] triangles = GenerateTriangles(m_MeshWidth, m_MeshHeight);
			m_Mesh.triangles = triangles;
