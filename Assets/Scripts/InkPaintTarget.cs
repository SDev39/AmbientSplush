...
public class InkPaintTarget : MonoBehaviour
{
     /// <summary>
    ...
    private void InitializeMesh()
    {
		...
		float uOffset = 1.0f / (height - 1);
		float vOffset = 1.0f / (width - 1);
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            {
				Vector3 uv = new Vector2(vOffset * x, uOffset * y);
                // Vector3 v = new Vector3(x * 0.01f, -y * 0.01f, 0) + k_DefaultMeshOffset;
				Vector3 v = DepthSource.GetVertexInWorldSpaceFromScreenXY(x, -y);
				if (maxX < v.x) maxX = (int)v.x;
				if (maxY < v.y) maxY = (int)v.y;
				if (maxZ < v.z) maxZ = (int)v.z;
                vertices.Add(v);
				uvs.Add(uv);
        ...
        m_Mesh.bounds = new Bounds(Vector3.zero, new Vector3(maxX, maxY, maxZ));
